...
> **Note:** the current release is a pre-1.0 alpha (`1.0.0a1`). Plain
> `pip install matia-client` / `poetry add matia-client` will not pull it in,
> since package managers skip pre-releases by default. Use
> `pip install --pre matia-client` or pin the exact version
> (`pip install matia-client==1.0.0a1`) to try it.
